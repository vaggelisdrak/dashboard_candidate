...
export default function SubmenuAccordion(props) {
          ...
          <li className={`${getNavLinkClass( menuitem.url ) ? 'relative active' : 'relative'}`}>
            ...
            <ul aria-expanded={ariaExpanded}  className={`${ showContent ? '!block ': ''}cpt hidden rounded rounded-t-none top-full z-50 ltr:pl-7 rtl:pr-7 py-0.5 ltr:text-left rtl:text-right mb-1 font-normal`}>
              {menuitem.submenu.map(submenuitem => {
                return<li key={`si-${submenuitem.id}`}><NavLink to={submenuitem.url} className={({ isActive }) => isActive ? 'text-indigo-600 block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 dark:hover:text-gray-300' : 'block w-full py-2 px-6 clear-both whitespace-nowrap hover:text-indigo-500 dark:hover:text-gray-300'}>{submenuitem.title}</NavLink></li>;
              })}
            </ul>
          </li>
        ) : (
          <li>
            <NavLink onClick={Toggler} to={menuitem.url} className={({ isActive }) => isActive ? 'text-indigo-600 block py-2.5 px-6 hover:text-indigo-500 dark:hover:text-gray-300' : 'block py-2.5 px-6 hover:text-indigo-500 dark:hover:text-gray-300'} >
